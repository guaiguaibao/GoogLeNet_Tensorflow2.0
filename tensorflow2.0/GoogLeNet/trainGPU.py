...
for epoch in range(1, epochs+1):
    ...
    # 这里的time.perf_counter()函数和time.time()函数作用类似
    ...
    """
    下面是cpu训练的一部分，不知道为什么cpu训练脚本和gpu训练脚本中，执行train_step和test_step这一部分不太一样，感觉没什么区别
    ...
    if test_loss.result() < best_test_loss:
        ...
        model.save_weights("./save_weights/myGoogLeNet_{}.ckpt".format(epoch), save_format='tf')
        # 用gpu训练的模型，最后将参数保存成ckpt格式的文件
